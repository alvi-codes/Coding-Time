...
// Counting Leaves
int count_leaves(TreeNode* t){
  if (t->left == NULL   &&   t->right == NULL){
    return 1; 
    // both sides are null, so it is a leaf
  }
  else if (t->right != NULL  &&  t->left == NULL){
    return count_leaves(t->right); 
    // left side is null, so we go right
  }
  else if (t->right == NULL && t->left != NULL){
    return count_leaves(t->left); 
    // right side is null, so we go left
  }
  else{ 
    return count_leaves(t-> left) + count_leaves(t-> right); 
    // none of the sides are null, so we both ways
  }
}
...
int main(){
    ...
    std::cout<<"\nNumber of LEAVES in the tree: "<< count_leaves(t) <<std::endl;
